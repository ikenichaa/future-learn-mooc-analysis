...
## 1.3 Determine Data Mining Goals
The action made by the learners when studying the online courses must be explored to see the pattern that the successful learner archetype has in common. The action chosen to be determined is the time spent to finish a 1-week study material of the online course. As the only way to complete the online course is to finish each small step daily, seeing how long the learner spent to finish each weekly module is a good start to knowing the characteristics of learner archetype approach to finish the online material.

Therefore, the main research topic for the second cycle is:
**Does the successful learner in each archetype have the same approach in time spent on learning 1-week study material?**

If there is the same pattern of success within an archetype, other students can follow the study pattern of the learner with the same archetype to finish the online course successfully.
...
## 2.1 Explore data
In this phase, let's explore the time to complete sub moule without taking a look at the archetype yet. The activity step table has columns *first_visited_at* and *last_completed_at* for each steps in the course. When grouping the week number and learner id together, the duration that each learner take to complete the weekly module in each course can be calculated. The *Figure 3* below calculated the duration in days that the successful learner, the learner with more than 80% course completion rate, took to complete the weekly material in run 3 to 7. Run 1 and 2 are ignore because they don't have survey response on archetype that will be used to analyse later on.

Ignoring the outlier from the plot, *Figure 3* shows the trend that most learner that complete the course finished the weekly module within a week. Additionally, most people spend less time in the latter week module. This exploration is promising in a way that it shows the pattern that finishing the course within the online course recommended timeframe can boost the online course completion rate.
...
boxplot_weekly_module_plot <- weekly_module_time_spent_with_arch |>
    ...
    labs(x = "week number", y = "duration in days")

# Annotate figure
annotate_figure(
  boxplot_weekly_module_plot, 
  top = text_grob(
          "Box plot shows duration the learner spend to learn each weekly module of the course", 
          face = "bold", 
          size = 12, 
          hjust = 0, 
          vjust = 0.4, 
...
# 3. Data Preparation
## 3.1 Select data
The CRISP-DM cycle 2 is built on top of the cycle 1, so the data in the first cycle will be used in this round too.
