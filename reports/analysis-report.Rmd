...
**Does the learner archetype affect online course success rate?**

## 1.4 Determine Data Mining Goals
If the archetype is significant to the MOOC finish rate, there should be a huge difference in the archetype of finisher and those who unable to finish the course. However, if the archetype is quite similar in both groups, it cannot be deduce that the archetype has an effect on the course completion.
...
1. Activity Step: `r colnames(cyber.security.1_step.activity)`
...
As right now we have all data prepared, we will examine our main research topic for this round, **Does the learner archetype affect online course success rate?**
...
As mentioned before, not many people response to the survey of the archetype, so the majority of learner archetype is unknown. If we ignore unknown type, depends on each run, some archetype perform better than the others. For a better understanding, the data is scaled for each run, so that the archetypes can be compare properly within the run. The brighter the color on the heatmap is the more number of successful learners in that archetype.
...
heatmap_data_success_rate <- success_rate_for_learner_archetype |>
...
Contrary to the previous result, the *flourisher* type has the highest completion rate among all archetypes in 3 out of 5 runs. The result can be seen clearer in the heatmap below.
```{r, echo=FALSE, out.width="70%", fig.align='center', fig.cap="Heatmap showing completion rate of learners by archetype"}
success_rate_plot <- ggplot(heatmap_data_success_rate, aes(x = Run, y = archetype, fill = percent)) +
    geom_tile() +
    scale_fill_viridis_c(option = "A") +
    theme_minimal() +
    labs(x = "run")
...
annotate_figure(
  success_rate_plot, 
  top = text_grob(
          "Heatmap showing completion rate of learners by archetype", 
          face = "bold", 
          size = 12, 
          hjust = 0, 
          vjust = 0.4, 
...
Interestingly, heatmap shows that *other* type is the only archetype that does not have the completion rate below 50%.

## 5 Evaluation
## 5.1 Evaluate Results
After explore the data on archetype and course completion rate thoroughly, the archetype has no prominent effect on the completion. At first glance, some archetype is standing out to have more number of complete learner but actually that is happening because there are more learner in that archetype as a whole so there is a higher chance for learners in that archetype to finish the course more. When comparing the success rate for each archetype, the result is too random in all archetype to have a significant impact on the course completion. Based on the findings, the reason to start a course and the archetype does not affect whether or not learner finish the online course.

## 5.2 Review Process
Data wrangling and data exploration have been revisited. The results provided above are legit.
TODO: talk about data quality that the survey response is too less.

## 5.3 Determine next steps
It is known that the type of learner cannot use to determined the completion rate, so the next step would be to find other features that contribute to the success in online course completion.
