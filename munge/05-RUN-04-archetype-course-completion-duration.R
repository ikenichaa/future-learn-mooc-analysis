...
activity_4_study_duration <- cyber.security.4_step.activity |>
...
steps_in_course_4 <- cyber.security.4_step.activity |>
...
find_steps_per_week <- function(week) {
    steps_per_week <- steps_in_course_4 |> filter(week_number == week) |> select(step_count) |> pull(step_count)

    return (steps_per_week)
}

# Get the number of steps each learner done in each courses' week
activity_4_steps_finish_per_week <- cyber.security.4_step.activity |>
    group_by(learner_id, week_number) |>
    count(learner_id) |>
    rename(steps_finished = 3)

# Add total number of steps per week to the data frame
activity_4_steps_finish_per_week['steps_per_week'] <- sapply(activity_4_steps_finish_per_week$week_number, find_steps_per_week)

# Calculate the percentage of steps finished
activity_4_steps_finish_per_week['percent_of_steps_finished'] <-
    round(activity_4_steps_finish_per_week$steps_finished/activity_4_steps_finish_per_week$steps_per_week * 100, 2)

# Retrieve only the course completer
activity_4_completer <- activity_4_steps_finish_per_week |>
    filter(percent_of_steps_finished > 80) |>
    select(learner_id, week_number, percent_of_steps_finished)

# Merge with duration_week_a4
# and drop rows with NA out because don't have last completed
activity_4_completer_study_duration <- activity_4_completer |>
    merge(y = activity_4_study_duration, by=c("week_number", "learner_id"), all.x=TRUE) |>
    na.omit()

activity_4_completer_study_duration_with_archetype <-
    activity_4_completer_study_duration |>
    merge(y=activity_4_learner_progress_archetype, by="learner_id", all.x=TRUE) |>
